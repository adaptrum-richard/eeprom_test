...
static ioctl_st *ioctl_st_init(void)
{
    ...
    iocs->msgs[0].buf = malloc(EEPROM_PAGE_SIZE + 2);
    if (!(iocs->msgs[0].buf)) {
...
int eeprom_page_read(u16 pos, u8 *data, int size)
{
    int fd;
    u8 *buf = data;
    eeprom_st eeps;
...
static void page_write_st_gen(ioctl_st *iocs, eeprom_st *eeps)
{
    ...
    iocs->msgs[0].addr = eeps->slave_addr;  //填入slave_addr
    iocs->msgs[0].flags = I2C_M_WR;         //write标志
    iocs->msgs[0].len = size + 2; //信号长度: 待写入数据长度 + byte_addr长度
    iocs->msgs[0].buf[0] = (eeps->byte_addr >> 8) & 0xff; //第1字节为byte_addr
    iocs->msgs[0].buf[1] = eeps->byte_addr & 0xff; //第2字节为byte_addr
    memcpy((iocs->msgs[0].buf + 2), eeps->buf, size); //copy待写数据
}
...
int eeprom_page_write(u16 pos, u8 *data, int size)
{
    int fd;
    u8 *buf = data;
    eeprom_st eeps;
...
/* 将1个字节数据写入eeprom的pos位置
   @pos: eeprom的byte_address
   @data: 待写入的字节数据 */
int eeprom_byte_write(u16 pos, u8 data)
{
    int ret;
    u8 buf = data;
